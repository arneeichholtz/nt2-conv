...
import subprocess       ##!! LB: to replace obsolete calls to ffmpeg  !!##
...
#        In our case, single_test=True 
        ...
        # model
        # by default add_audio_in = 
        # I do not understand why that is the case. But add_audio_in is indeed set to False
        ...
        if (opt_parser.load_G_name != ''):
            ckpt = torch.load(opt_parser.load_G_name, weights_only=False)
            try:
    ...
    def test(self):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            xx = str('ffmpeg -y -i tmp_{:04d}.mp4 -pix_fmt yuv420p random_{:04d}.mp4'.format(i, i))
            xxx = xx.split()

            code = subprocess.run(xxx)
            filename = str('tmp_{:04d}.mp4'.format(i))
            os.remove(filename)
    ...
    def single_test(self, jpg=None, fls=None, filename=None, prefix='', grey_only=False):
        ...
        if(fls is None):
            ...
            fls[:, 1::3] += 80
        """
        myfls = np.ndarray((fls.shape[0]//3, fls.shape[1], fls.shape[2]), float)
        pointer = 0
        for jj in range(0, fls.shape[0]-2, 3):
            myfls[pointer, :, :] = fls[jj, :, :]
            pointer += 1
        """
...
#       print(filename, ref_in.shape, g_out.shape, fls_in.shape, frame.shape)
        ...
        xxx.append('-pix_fmt')
        xxx.append('yuv420p')
        xxx.append(filename[:-4].replace('pred_fls', self.opt_parser.jpg[:-4]) + '.mp4')
    #    print(xxx)
        code = subprocess.run(xxx)
