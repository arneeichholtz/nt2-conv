...
class Audio2landmark_model():
    ...
    def __init__(self, jpg_shape=None):
        '''
        ...
        self.load_a2l_G_name = 'examples/ckpt/ckpt_speaker_branch.pth'
        self.load_a2l_C_name = 'examples/ckpt/ckpt_content_branch.pth'
...
#        print('======== LOAD PRETRAINED FACE ID MODEL {} ========='.format(opt_parser.load_a2l_C_name))
    ...
    def __train_face_and_pos__(self, fls, aus, embs, face_id, smooth_win=31, close_mouth_ratio=.99):
        ...
        z = torch.zeros(aus.shape[0], 128).detach().clone().to(device)
#        z = torch.tensor(torch.zeros(aus.shape[0], 128), requires_grad=False, dtype=torch.float).to(device)
    ...
    #
        ...
        fl_dis_pred = torch.tensor(fl_dis_pred).to(device) * self.amp_pos
    ...
    def __calib_baseline_pred_fls_old_(self, baseline_pred_fls, residual_face_id, aus):
        ...
        baseline_pred_fls[:, 48 * 3::3] *= self.amp_lip_x  # mouth x
        baseline_pred_fls[:, 48 * 3 + 1::3] *= self.amp_lip_y  # mouth y
    ...
    def __train_pass__(self, au_emb=None, centerize_face=False, no_y_rotation=False, vis_fls=False):
        ...
        for i, batch in enumerate(dataloader):
            ...
            ## print('audio2landmark, inputs_fl, inputs_au, inputs_emb', inputs_fl.shape, inputs_au.shape, inputs_emb.shape)
...
#               keys = ['audio_embed']
            ...
            for key in keys: # ['45hn7-LXDX8']: #['sxCbrYjBsGA']:#
                ...
                seg_bs = 512
                ## print('inputs_fl', inputs_fl.shape)
                ...
                ## print('fake_fls_np', fake_fls_np.shape)
