...
def extract_f0_func_audiofile(audio_file, gender='M'):
    ...
    return S, f0_norm, D_mel


def extract_f0_func_wav(x, gender='F'):
    floor_sp, ceil_sp = -80, 30
    mel_basis = mel(sr=16000, n_fft=1024, fmin=90, fmax=7600, n_mels=80).T
    min_level = np.exp(-100 / 20 * np.log(10))
    b, a = butter_highpass(30, 16000, order=5)
...
##    x, fs = sf.read(audio_file)
    ...
    y = signal.filtfilt(b, a, x)
    wav = y * 0.95 + (prng.rand(y.shape[0]) - 0.5) * 1e-06
    D = pySTFT(wav, fft_length=1024, hop_length=3*256).T
    D_mel = np.dot(D, mel_basis)
    D_db = 20 * np.log10(np.maximum(min_level, D_mel)) - 16
    S = (D_db + 100) / 100
#
# sptk.rapt is the f0 computation.
# the 3rd argument is the hop size, which determines the effective sampling frquency.
#
    f0_rapt = sptk.rapt(wav.astype(np.float32) * 32768, 16000, 3*256, min=lo, max=hi, otype=2)
    index_nonzero = (f0_rapt != -1e10)
    tmp = f0_rapt[index_nonzero]
    mean_f0, std_f0 = np.mean(tmp), np.std(tmp)
