"""
 ...
 # Replaced the python ffmpeg call in __init__ by the stand-alone ffmpeg.
 # It remains to be established wheter other parts of this file are used.
 # Commented out the class Vis_old()
"""
...
class Vis():
    ...
    def __init__(self, fls, filename, audio_filenam=None, fps=100, frames=1000):
        ...
        fls[:, 1::3] += 100
#
# This creates an mp4v file with the line drawing cartoon.
#
        ...
        writer.release()
#
# The following code adds the audio to the line drawing cartoon.
#
        if(audio_filenam is not None):

            xxx = []
            xxx.append('ffmpeg')
            xxx.append('-hide_banner')
            xxx.append('-y')
            xxx.append('-loglevel')
            xxx.append('error')
            xxx.append('-i')
            xxx.append(audio_filenam)
            xxx.append('-i')
            xxx.append('examples/tmp.mp4')
            xxx.append('examples/' + filename + '_av.mp4')
            ...
            print('ffmpeg -y -i {} -i {} -strict -2 -shortest {}'.format(
                os.path.join('examples', 'tmp.mp4'),
                audio_filenam,
                os.path.join('examples', '{}_av.mp4'.format(filename)))
            )
